...
//        }
    ...
    public void moveOnYAxis(int ticks){
        ...
        while (opMode.opModeIsActive() && frontLeft.isBusy()) {
            ...
            opMode.telemetry.addData("Front Left Encoder P", frontLeft.getPower());
            opMode.telemetry.addData("Front Right Encoder", frontRight.getPower());
            opMode.telemetry.addData("Rear Left Encoder", rearLeft.getPower());
            opMode.telemetry.addData("Rear Right Encoder", rearRight.getPower());
            opMode.telemetry.addData("Front Left Encoder P", frontLeft.getVelocity());
            opMode.telemetry.addData("Front Right Encoder", frontRight.getVelocity());
            opMode.telemetry.addData("Rear Left Encoder", rearLeft.getVelocity());
            opMode.telemetry.addData("Rear Right Encoder", rearRight.getVelocity());
            opMode.telemetry.update();
        }
    ...
    // complete !!!!
    public void autoMiddle(int isBlue){
        // flip claw down
    ...
    // left: -1
    // right: 1
    public void autoAwayFromTruss(int isBlue){
        // spin 90 degrees in a direction depending on which side we're on
        ...
        this.moveOnXAxis(this.inchesToCounts(18*isBlue));
        // move to panel forwards
        this.moveOnYAxis(this.inchesToCounts(26));
        // spin 90 degrees in a direction depending on which side we're on
        this.Spin(this.inchesToCounts(18*-isBlue));
    }
    ...
    // left: -1
    // right: 1
    public void autoCloseToTruss(int isBlue){
        // spin 90 degrees in a direction depending on which side we're on
        this.Spin(this.inchesToCounts(18*isBlue));
        // flip claw down
        WristL.setPosition(0.95);
    ...
    public void scoreBackboard(int backPosition, int isBlue){
        opMode.sleep(400);
        // move back to the backboard
        this.moveOnYAxis(RoboController.inchesToCounts(-45));
        // reposition on board
